...
server <-
  function(input, output, session) {
        ...
        if (input$chat_mode) {
          ...
          temp_hospitals <-
            ...
            filter(
              ...
              Excess >= min(input$excess),
              Excess <= max(input$excess),
              Predicted >= min(input$predicted),
              Predicted <= max(input$predicted),
              Expected >= min(input$expected),
              Expected <= max(input$expected)
            )
        }
      ...
      leafletProxy("hospital_map") |>
        ...
        addCircleMarkers(
          ...
          popup = ~ paste0(
            ...
            "<br>Diagnosis Group: ",
            DiagnosisCategory,
            "<br>Excess Readmission Ratio: ",
            Excess,
            "<br>Predicted Readmission Rate: ",
            round(Predicted, 2),
            "%",
            "<br>Expected Readmission Rate: ",
            round(Expected, 2),
            "%"
          ),
          color = ~ pal(-1 * Excess),
    ...
    output$what_diagnosis <- renderText({
      paste0(
        "Focus Diagnosis Group: ",
        paste(
          sort(unique(current_hospitals()$DiagnosisCategory)),
          collapse = ", "
        )
      )
    })
    ...
    output$excess_count <-
      renderUI({
        ...
        temp_count <- paste0(
          temp_count,
          " (",
          round(100 * temp_count / n_distinct(current_hospitals()$FacilityID)),
          "%)"
        )
    ...
    output$hospital_table <- DT::renderDataTable({
      current_hospitals() |>
        rename(
          ...
          `Excess Readmission Ratio` = Excess,
